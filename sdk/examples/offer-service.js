...
async function main() {
    ...
    const pricePerUnit = ethers_1.ethers.parseEther('0.0001');
    try {
        console.log('\n📝 Registering service...');
        console.log('Type: sentiment-analysis');
        console.log('Price:', ethers_1.ethers.formatEther(pricePerUnit), 'ETH per 1000 tokens');
        const serviceId = await client.offerService('sentiment-analysis', pricePerUnit, metadataURI);
        ...
        for (const service of services) {
            ...
            console.log(`  Price: ${ethers_1.ethers.formatEther(service.pricePerUnit)} ETH/unit`);
        ...
        await client.listenForOrders(async (orderId, order) => {
            ...
            console.log(`Total Price: ${ethers_1.ethers.formatEther(order.totalPrice)} ETH`);
            ...
            const resultHash = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes(JSON.stringify(resultData)));
            await client.completeOrder(orderId, resultURI, ethers_1.ethers.getBytes(resultHash));
